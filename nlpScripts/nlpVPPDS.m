...
function [varS, problem, stateEnd] =...
    nlpVPPDS(varS, dynS, problem, i, stateK)
    ...
    for k = varS.params.shootCount:...
        ...
        if (k ~= (varS.params.shootCount + varS.params.N - 1))
            ...
            % Store hip position
            xHip = stateK(1) - varS.params.rH*sin(stateK(3));
            yHip = stateK(2) - varS.params.rH*cos(stateK(3));

            % Calculate lag leg length
            lenLag = sqrt((xHip - varS.var.(['footDLag' num2str(i)]))^2 +...
                yHip^2);

            % Constraint: Force along lag leg must never be less than 0
            [problem] = addConstraint(problem,...
                1000*(stateK(7)*(varS.var.len0 - lenLag) +...
